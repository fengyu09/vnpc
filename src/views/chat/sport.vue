<template>
  <div id="chat">
    ...
    <div class="video-room clearfix">
      <!--播放器-->
      <div class="video-container" :style="!lts ? 'width:100%' : '958px'">
        <!-- <div class="video-top clearfix">
          <dl class="fl clearfix">
            <dt class="fl" v-if="liveData">
              <router-link target="_blank" :to="'/HostHome/' + liveData.anchor_id">
                <img v-lazy="liveData.avatar" alt />
              </router-link>
            </dt>
            <dd class="fl" v-if="liveData">
              <h3 class="nick-name">{{ liveData.nickname }} <i class="tj-zb" v-if="$store.state.codeToken&&isHotPut!=0&&$store.state.userinfo.noble>6" @click="tjfunc(liveData.anchor_id)"></i><i class="tj-zb tj-zb-over" v-if="$store.state.codeToken&&isHotPut==0&&$store.state.userinfo.noble>6" @click="tjfuncOver()"></i></h3>
              <p>
                <span class="online-room">房间ID：{{liveData.anchor_id}}</span>
                <span class="online-num">{{ liveData.online }}</span>
                <span
                  class="online-time"
                  v-if="liveData.liveStartTimeTxt && liveData.liveEndTimeTxt"
                >直播时间：{{ liveData.liveStartTime | filterTime}}-{{liveData.liveEndTime | filterTime}}</span>
        ...
        <div class="video-content">
          ...
          <div
            class="no-open sport-living"
            v-if="liveData && !liveData.live_status"
          >
            <!-- 直播体育 -->
            <div class="game-living-box">
              <div class="living-title clearfix">
                <div class="fl host-box">
                  <div class="div match-img">
                    <img v-lazy="liveData.race_config.homelogo" alt="" />
                  </div>
                  <span>{{ liveData.race_config.homesxname }}（主）</span>
                  <span class="bf">{{ liveData.race_config.homescore }}</span>
                </div>
                <span class="match-status"
                  >{{ liveData.race_config.simpleleague }}
                  <i v-if="liveData.race_config.stagename"
                    >({{ liveData.race_config.stagename }})</i
                  >
                  {{ liveData.race_config.matchtime }} <br />{{
                    liveData.race_config.status_desc
                  }}</span
                >
                <div class="fr kd-box">
                  <span class="bf">{{ liveData.race_config.awayscore }}</span>
                  <span>{{ liveData.race_config.awaysxname }}（客）</span>
                  <div class="div match-img">
                    <img v-lazy="liveData.race_config.awaylogo" alt="" />
                  </div>
                </div>
              </div>
              ...
              <div class="living-box">
                ...
                <video-player
                  ...
                  @waiting="waiting()"
                  v-show="
                    liveData && liveData.live_status && isShowRtmp == 'noFire'
                  "
                />
                <div
                  class="no-live-box"
                  v-show="liveData && !liveData.live_status"
                ></div>
              </div>
              <!-- <div class="living-bottom clearfix">
                <div class="fl host">
                  <div class="item">
                    <span class="fl" style="padding-right:8px;">59%</span>
                    <div class="progress fl"><div class="curr-progress orange"></div></div>
                    <span  class="fl text">控球率</span>
                    <div class="progress fl" ><div class="curr-progress blue"></div></div>
                    <span class="fl" style="padding-left:15px">33%</span>
                  </div>
        ...
        <div class="git-box" @mouseleave="gitMsg = false">
          ...
          <div
            :class="hoverGitMsg.grade=='low' ?  'show-img-msg':'show-img-msg3'"
            v-show="gitMsg"
            class="show-img-msg4"
          >
            <dl class="clearfix">
          ...
          <!-- <div class="first-charge-msg">
            <div class="first-charge">
              <img
                ...
                @click="showRecharge"
              />
              <img style="margin-left:20px"  @click="openDZP()" src="../../assets/imgaes/chat/dzp.png" alt="">
            </div>
            <div class="git-msg">
              <span>
      ...
      <!--聊天室-->
      <div class="fr fr-container sport-container" v-if="lts">
        <div class="fr-tab-title">
          phòng chat
          <!-- <span  @click="Chatroom()"
                       :class="currType == 1 ? 'active' : ''">聊天室</span> -->
          <!-- <span  @click="Jingcai()"
                      :class="currType == 2 ? 'active' : ''">竞猜</span> -->
        </div>
        <div
          class="fr chat-container"
          v-show="currType == 1"
          style="height: 729px"
        >
          <!--普通用户 游客进入直播间提示-->
                        ...
                        <ul v-show="bdList">
                            <li v-for="(item, index) in bdList" :key="index">
                                <router-link :to="'/userMsg/' + item.user_id">
                                    ...
                                    <img class="No1" v-if="index==0" src="../../assets/imgaes/chat/01-min.png" alt />
                                    <img class="No1" v-if="index==1" src="../../assets/imgaes/chat/02-min.png" alt />
                                    <img class="No1" v-if="index==2" src="../../assets/imgaes/chat/03-min.png" alt />
                                </router-link>
                            </li>
                        </ul>
                    </div>
                    <div class="left-btn bd-btn" @click="goLeft" v-if="isShowbL"></div>
                    <div class="right-btn bd-btn" @click="goRight" v-if="isShowbR"></div>
                </div> -->
          <el-scrollbar
            style="
              height: 596px;
          ...
          >
            <p class="system-tip">
              Mẹo hệ thống: K188 cố gắng tạo ra một môi trường phát sóng
              trực tiếp tích cực và lành mạnh, đồng thời tiến hành kiểm tra nội
              dung phát sóng trực tiếp 24 giờ. Một khi bất kỳ hành vi truyền bá
              bất hợp pháp, bất hợp pháp, gian lận, thô tục hoặc bạo lực đẫm máu
              bị phát hiện, tài khoản của người vi phạm sẽ bị cấm.
            </p>
            <span
              v-if="!chatData"
              ...
              >聊天室loading...</span
            >
            <ul class="chat-content" v-if="chatData">
                ...
                >
                  ...
                  <div class="chitchat fl">
                    ...
                    <ul class="table height">
                      <li class="title">
                        <span>玩法</span><span style="width: 22%">号码</span
          ...
          <div class="footer-input">
            ...
            >
              <input
                ...
                ref="focusTextarea"
                type="text"
                placeholder="Tôi cũng nói một vài từ"
          ...
          <!-- <keep-alive>
            <vme v-if="showEmj" ref="vmeChild"></vme>
          </keep-alive> -->
          <div class="cl-box">
        ...
        >
          ...
          >
            <div class="prevue-l" v-if="LivePreviewNew.length">
              ...
              <span class="prevue-line p-line2" v-if="LivePreviewNew.length > 3"></span>
              <span class="prevue-line p-line1" v-if="LivePreviewNew.length > 2"></span>
              <el-scrollbar style="height:339px;">
                <ul class="clearfix">
                  <li class="clearfix" v-for="(ii, index) in LivePreviewNew" :key="index">
                    <div class="clearfix">
                      ...
                      <div class="host-details fl">
                        <div class="anchor-name clearfix">
                          ...
                          <span class="anchor-icon" v-else>
                            <img src="../../assets/imgaes/main/sy_rmzb_qc@3x.png" alt />
                          </span>
                          <span
                            class="anchor-title"
                        ...
                        <div class="anchor-game">{{ ii.name ? ii.name : "" }}</div>
                        <div
                          class="anchor-time"
                          v-if="ii.starttime && ii.endtime"
                        >{{ ii.starttime }} - {{ ii.endtime }}</div>
                      ...
                      >{{ ii.isFollow == false ? "关注" : "已关注" }}</div>
                    </div>
                  </li>
                </ul>
              </el-scrollbar>
            </div>
            <div class="prevue-no" v-if="!LivePreviewNew.length">暂时还没有预告哟~</div>
          </el-tab-pane>
        </el-tabs>
      </div>
    ...
    <!-- 校验是否设置支付密码-->
    <div class="fa-redpack" v-if="fPsd">
...
.game-living-box {
  ...
  .living-title {
    ...
    &::before {
      ...
      width: 175px;
      height: 80px;
      background: url(../../assets/imgaes/match/hb.png) no-repeat;
    ...
    &::after {
      ...
      width: 175px;
      height: 80px;
      background: url(../../assets/imgaes/match/lb.png) no-repeat;
      background-size: contain;
    }
    > div {
      position: relative;
  ...
  .living-box {
    //  background: url(../../assets/imgaes/match/dhbj.png) center center no-repeat;
    // // background: url(../../assets/images/match/dhbj.png) center center no-repeat;
