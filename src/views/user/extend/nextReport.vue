  ...
  <div class="next-report">
    ...
    <div class="team-title">
      <ul class="clearfix">
        <li>
          ...
          <span>{{$t('extendReport.nextReport.text[0]')}}</span>
        </li>
        <li>
          <span>{{ lastMsg.recharge }}</span>
          <span>{{$t('extendReport.nextReport.text[1]')}}</span>
        </li>
        <li>
          <span>{{ lastMsg.brokerage }}</span>
          <span>{{$t('extendReport.nextReport.text[2]')}}</span>
        </li>
        <li class="dw">
          <p style="padding-bottom: 6px">
           {{$t('extendReport.nextReport.text[3]')}} {{ currCodeMsg.rebate * 100 }}%
          </p>
          <div class="dw-bl">
          ...
          <p v-if="isInvite" style="padding-top: 14px">
            {{ currCodeMsg.next_level_diff }} {{$t('extendReport.nextReport.text[4]')}}
          </p>
          <p v-if="!isInvite" style="padding-top: 14px">
    ...
    <div class="search-box">
      <input
        ...
        :placeholder=" $t('extendReport.nextReport.text[5]')"
        @input="searchInput($event)"
      />
      <button :class="[{'pink-bg':skin==1},{'blue-bg':skin==2},{'green-bg':skin==3}]" class="search-btn" @click="searchBtn"> {{$t('extendReport.nextReport.text[6]')}}</button>
    </div>
    <div class="re-content">
      <div class="title">
        <span class="user-pic" > {{$t('extendReport.nextReport.tableTitle[0]')}}</span>
        <span class="name">{{$t('extendReport.nextReport.tableTitle[1]')}}</span>
        <span class="">{{$t('extendReport.nextReport.tableTitle[2]')}}</span>
        <span class="">{{$t('extendReport.nextReport.tableTitle[3]')}}</span>
        <span class="">{{$t('extendReport.nextReport.tableTitle[4]')}}</span>
      </div>
      <el-scrollbar
        style="
          height: 400px;
      ...
      >
        <div class="item" v-for="(v, index) in reportList" :key="index">
          <span class="user-pic "><img v-lazy="v.avatar" alt="" /></span>
          <span class="name ">
            <i>{{ v.nickname }}</i>
             <!-- <img :src="v.level" alt="" /> -->
            <!-- <i
              v-if="v.level"
              class="dw-icon level-icon"
              :class="v.level | filterLevel"
            ></i> -->
          </span>
          <span class="w20">{{ v.invitee_num }}</span>
          <span class="w20">{{ v.recharge }}</span>
          <!-- <span style="display: none">{{ v.exchange }}</span> -->
          <span class="bkge w20">{{ v.brokerage }}</span>
        </div>
        <el-pagination
          v-if="total > 10"
        ...
        >
          <!-- <p>loading</p> -->
          <p class="huiColor"> {{$t('extendReport.nextReport.text[7]')}}</p>
        </div>
      </el-scrollbar>
    </div>
  </div>
  ...
  methods: {
    ...
    searchBtn: function () {
      ...
      if (this.searchContent === "" || this.searchContent == null) {
        this.$message.error(this.$t('extendReport.nextReport.warnText[0]'));
        return;
      }
