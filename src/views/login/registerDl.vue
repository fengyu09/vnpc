...
export default {
  ...
  methods: {
    ...
    async register() {
      ...
      if (!this.regForm.extendCode) {
        this.$message({
          message: this.$t('register.warnText[3]'),
          type: "warning",
        });
        this.$refs.extendCode.focus();
        return;
      }
      if (this.isCaptcha && !this.regForm.captcha) {
        this.$message({
          message: this.$t('register.pla[4]'),
          type: "warning",
        });
    ...
    phoneverify(e) {
      this.regForm.phone = e.target.value.replace(/[^\d]/g, "");
    },
