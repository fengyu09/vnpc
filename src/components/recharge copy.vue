        ...
        <div class="recharge-content" :class="isYHK == true ?'isyhk':''" ref="rechargeScroll">
          ...
          <div class="recharge-type" v-if="!isYHK">
            ...
            <div class="recharge-type" v-if="$st.state.czTypeArr.length">
              <!-- 充值方式 -->
              <ul class="type-items" :class="[{'qd-items-pink':skin==1},{'qd-items-blue':skin==2},{'qd-items-green':skin==3}]">
              ...
              <!-- 充值渠道 -->
              <ul class="qd-items" v-if="currType!=2" :class="[{'qdd-items-pink':skin==1},{'qdd-items-blue':skin==2},{'qdd-items-green':skin==3}]">
          ...
          <div class="xnb-xy" v-if="isXnb">
              ...
              <div class="virtual-xy">
                <span v-for="(item,index) in codeList" :class="{active:virtualCurrId == item.protocol}" @click="selectWallet(item)" :key='index'>{{item.protocol}}</span>
              </div>
              <p>Vui lòng quét mã QR hoặc sao chép liên kết với ví USDT để hoàn tất thanh toán</p>

             <div class="recharge-value">
                      <h3>Số tiền nạp<div class="p fr"><router-link  tag="a" target="_blank" :class="[{'pink-color':skin==1},{'blue-color':skin==2},{'green-color':skin==3}]"   style="font-weight: normal; font-size: 14px;color:#4BA1F9" to="/Tutorials">{{$t('recharge.text[4]')}}</router-link></div></h3>
                      ...
                      <div class="other-money">
                        <input type="text" maxlength="10" @input="shuru" v-model="rechargeData.amount" placeholder="请输入金额"> VNDK
                      </div>
                    </div>
          ...
          <div class="recharge-value" v-if="!isYHK&& !isSdXnb&&!isXnb&&$st.state.czTypeArr.length&&currqd>0" >
            ...
            <div class="bank-code-select" v-if="isBankCode">
               <el-select @change="pickerConfirm" v-model="bankCodeselectSort" clearable placeholder="Chọn ngân hàng">
                  <el-option
          ...
          <div class="yhk-box" v-show="isYHK">
            ...
            <br>
            <div class="yhk-bd">
              <span class="card-title">{{$t('recharge.sdyhk.inputTitle[2]')}}:</span>
              <br>
              <span class="card-content">{{czCardInfo.bank}}</span>
              <span class="copy-btn copy-bank" @click="copyLinkCard('copy-bank')" data-clipboard-action="copy" :data-clipboard-text="czCardInfo.bank">{{$t('copyText')}}</span>
            </div>
            <p class="yhk-bt">3.  {{$t('recharge.sdyhk.text[6]')}}: <button class="cyzh-btn" @click="cyzhOpen"> {{$t('recharge.sdyhk.text[7]')}}</button></p>
            ...
            <div class="yhk-bd">
             ...
             <div class="yhk-bd">
                ...
                <el-select @change="platformChanged" v-model="selectSort" clearable :placeholder="$t('recharge.sdyhk.pla[4]')" >
                  <el-option
